package com.taha.users.restcontrollers;

import java.util.List;

import com.taha.users.entities.Role;
...
import com.taha.users.service.UserService;
@RestController
@CrossOrigin(origins = "*")
public class UserRestController {
    ...
    public ResponseEntity<String> activateUser(@PathVariable String username,@PathVariable String code ) {

        System.out.println("user activated: " + code);
        User user =userService.activateUser(username, code);
        if (user != null) {
            // Assuming your frontend is running on http://localhost:4200
            String frontendUrl = "http://localhost:4200/verified"; // Change this URL accordingly
            ...
            return ResponseEntity.badRequest().body("Go to sleep");
        }
